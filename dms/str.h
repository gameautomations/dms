...
inline std::string bstr_to_utf8(const _bstr_t &bstr) {
  ...
  WideCharToMultiByte(CP_UTF8, 0, bstr, len, ret.data(),
                      ret.size(), // NOLINT
                      nullptr, nullptr);
...
inline std::wstring utf8_to_utf16(const std::string &str) {
  ...
  const int chars_converted =
      ::MultiByteToWideChar(CP_UTF8, 0, str.data(), static_cast<int>(str.size()), &buffer[0],
                            buffer.size() // NOLINT
      );
...
inline std::string url_encode(const std::string &str) {
  ...
  for (size_t i = 0; i < length; i++) {
    if (isalnum(static_cast<unsigned char>(str[i])) || (str[i] == '-') || (str[i] == '_') ||
        (str[i] == '.') || (str[i] == '~'))
      str_temp += str[i];
    else if (str[i] == ' ')
      str_temp += "+";
...
inline _bstr_t encoded_uri_component_to_bstr(const std::string &encoded) {
  return {utf8_to_utf16(encoded).c_str()};
}

inline _bstr_t utf8_to_bstr(const std::string &encoded) {
  return {utf8_to_utf16(encoded).c_str()};
}
