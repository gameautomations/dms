...
Idmsoft *create_dm() {
  ...
  // 获得大漠 DLL 中的函数
  if (const auto fn_gco =
          reinterpret_cast<pfn_gco>(GetProcAddress(hdll_inst, "DllGetClassObject"));
      fn_gco != nullptr) {
    IClassFactory *class_factory = nullptr;

    if (HRESULT h_result = (fn_gco)(__uuidof(dmsoft), IID_IClassFactory,
                                    reinterpret_cast<void **>(&class_factory));
        SUCCEEDED(h_result) && (class_factory != nullptr)) {
      // 创建大漠 实例
      h_result = class_factory->CreateInstance(nullptr, __uuidof(Idmsoft),
                                               reinterpret_cast<void **>(&dm_obj));
